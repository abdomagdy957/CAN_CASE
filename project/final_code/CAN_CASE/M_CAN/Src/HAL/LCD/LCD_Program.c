...
void LCD_voidSendCommand(u8 command){
	GPIO_voidSetPinValue(LCD_CONTROL_PORT,LCD_RS, GPIO_LOW); //RS = 0

	GPIO_voidSetPinValue(LCD_CONTROL_PORT, LCD_EN, GPIO_HIGH); //EN = 1

#if LCD_BITS == LCD_8_BIT_MODE
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D0, Get_Bit(command, 0));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D1, Get_Bit(command, 1));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D2, Get_Bit(command, 2));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D3, Get_Bit(command, 3));
#endif
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D4, Get_Bit(command, 4));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D5, Get_Bit(command, 5));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D6, Get_Bit(command, 6));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D7, Get_Bit(command, 7));
	GPIO_voidSetPinValue(LCD_CONTROL_PORT, LCD_EN, GPIO_LOW); //EN = 0

#if LCD_BITS == LCD_4_BIT_MODE
	GPIO_voidSetPinValue(LCD_CONTROL_PORT, LCD_EN, GPIO_HIGH); //EN = 1
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D4, Get_Bit(command, 0));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D5, Get_Bit(command, 1));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D6, Get_Bit(command, 2));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D7, Get_Bit(command, 3));
	GPIO_voidSetPinValue(LCD_CONTROL_PORT, LCD_EN, GPIO_LOW); //EN = 0
#endif
	_delay_ms(5);
}
...
void LCD_voidWriteChar(u8 data){
	GPIO_voidSetPinValue(LCD_CONTROL_PORT,LCD_RS, GPIO_HIGH); //RS = 0

	GPIO_voidSetPinValue(LCD_CONTROL_PORT, LCD_EN, GPIO_HIGH); //EN = 1
#if LCD_BITS == LCD_8_BIT_MODE
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D0, Get_Bit(data, 0));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D1, Get_Bit(data, 1));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D2, Get_Bit(data, 2));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D3, Get_Bit(data, 3));
#endif
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D4, Get_Bit(data, 4));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D5, Get_Bit(data, 5));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D6, Get_Bit(data, 6));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D7, Get_Bit(data, 7));
	GPIO_voidSetPinValue(LCD_CONTROL_PORT, LCD_EN, GPIO_LOW); //EN = 0

#if LCD_BITS == LCD_4_BIT_MODE
	GPIO_voidSetPinValue(LCD_CONTROL_PORT, LCD_EN, GPIO_HIGH); //EN = 1
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D4, Get_Bit(data, 0));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D5, Get_Bit(data, 1));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D6, Get_Bit(data, 2));
	GPIO_voidSetPinValue(LCD_DATA_PORT, LCD_D7, Get_Bit(data, 3));
	GPIO_voidSetPinValue(LCD_CONTROL_PORT, LCD_EN, GPIO_LOW); //EN = 0
#endif
	_delay_ms(5);
}
